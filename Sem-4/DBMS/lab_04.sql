...
Select dbo.OddEven(2) as Odd_Even
...
Returns table
AS
	return (Select * from Person where Person.FirstName LIKE 'B%')

Select * from PersonDetails() AS PersonName
...
Select * from PersonUnique() AS UniquePersonName
...
Select dbo.Compare(1,2)
Select dbo.Compare(5,1)
...
Returns table
AS
	return (Select * from Person where Person.PersonID = @ID)

Select * from PersonDetail(102) AS Person

--15. Write a function that returns a table with details of all persons who joined after 1-1-1991.
CREATE OR ALTER FUNCTION PersonDetailJoiningDate()
Returns table
AS
	return (Select * from Person where Person.JoiningDate>'1991-1-1')

Select * from PersonDetailJoiningDate() 